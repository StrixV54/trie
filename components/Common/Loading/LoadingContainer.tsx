...
const LoadingContainer = ({ enabled = true, title, icon, size = 'base', className = '', spinnerSpeed, border = false }: ILoadingProps) => {
  if (!enabled) return null
  const loadingBorder = border ? 'loading-border' : ''

  return (
    <div className={`loading-icon-container ${className} ${loadingBorder}`}>
      <div className={`loading-icon ${size}`}>{icon || <LoadingSpinnerIcon speed={spinnerSpeed} />}</div>
